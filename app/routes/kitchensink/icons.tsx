import { ContentContainer } from "~/components";
import Add from "~/components/icons/mui/Add";
import AddFile from "~/components/icons/mui/AddFile";
import AnnouncementOutlined from "~/components/icons/mui/AnnouncementOutlined";
import ArrowBack from "~/components/icons/mui/ArrowBack";
import ArrowRight from "~/components/icons/mui/ArrowRight";
import { ArrowRightLong } from "~/components/icons/mui/ArrowRightLong";
import Attention from "~/components/icons/mui/Attention";
import Bell from "~/components/icons/mui/Bell";
import Cancel from "~/components/icons/mui/Cancel";
import Check from "~/components/icons/mui/Check";
import Close from "~/components/icons/mui/Close";
import Communication from "~/components/icons/mui/Communication";
import DeleteOutline from "~/components/icons/mui/DeleteOutline";
import Download from "~/components/icons/mui/Download";
import Edit from "~/components/icons/mui/Edit";
import EditOutlined from "~/components/icons/mui/EditOutlined";
import EmailOutlined from "~/components/icons/mui/EmailOutlined";
import ExclamationMark from "~/components/icons/mui/ExclamationMark";
import ExclamationMarkFilled from "~/components/icons/mui/ExclamationMarkFilled";
import FileCopy from "~/components/icons/mui/FileCopy";
import Finished from "~/components/icons/mui/Finished";
import HelpOutline from "~/components/icons/mui/HelpOutline";
import HighlightOff from "~/components/icons/mui/HighlightOff";
import House from "~/components/icons/mui/House";
import Info from "~/components/icons/mui/Info";
import LetterIcon from "~/components/icons/mui/LetterIcon";
import Lock from "~/components/icons/mui/Lock";
import Logout from "~/components/icons/mui/Logout";
import Loop from "~/components/icons/mui/Loop";
import Menu from "~/components/icons/mui/Menu";
import Paragraph from "~/components/icons/mui/Paragraph";
import Person from "~/components/icons/mui/Person";
import PersonCircle from "~/components/icons/mui/PersonCircle";
import Plus from "~/components/icons/mui/Plus";
import QuestionMark from "~/components/icons/mui/QuestionMark";
import Refresh from "~/components/icons/mui/Refresh";
import Remove from "~/components/icons/mui/Remove";
import Send from "~/components/icons/mui/Send";
import Slash from "~/components/icons/mui/Slash";
import YellowArrowLeft from "~/components/icons/mui/YellowArrowLeft";
import OpenTab from "~/components/icons/mui/OpenTab";
import WarningAmber from "~/components/icons/mui/WarningAmber";
import Schedule from "~/components/icons/mui/Schedule";
import TaskAlt from "~/components/icons/mui/TaskAlt";

export default function KitchenSinkIcons() {
  return (
    <ContentContainer className="stack">
      <AddFile />
      <Add />
      <AnnouncementOutlined />
      <ArrowBack />
      <ArrowRightLong />
      <ArrowRight />
      <Attention />
      <Bell />
      <Cancel />
      <Check />
      <Close />
      <Communication />
      <DeleteOutline />
      <Download />
      <EditOutlined />
      <Edit />
      <EmailOutlined />
      <ExclamationMarkFilled />
      <ExclamationMark />
      <FileCopy />
      <Finished />
      <HelpOutline />
      <HighlightOff />
      <House />
      <Info />
      <LetterIcon />
      <Lock />
      <Logout />
      <Loop />
      <Menu />
      <OpenTab />
      <Paragraph />
      <PersonCircle />
      <Person />
      <Plus />
      <QuestionMark />
      <Refresh />
      <Remove />
      <Schedule />
      <Send />
      <Slash />
      <TaskAlt />
      <WarningAmber />
      <YellowArrowLeft />
    </ContentContainer>
  );
}
